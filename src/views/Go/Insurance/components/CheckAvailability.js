...
import { updateGoUser } from 'actions/api';
...
const CheckAvailability = (props) => {
  const { user, updateGoUser, nextStep, backTab } = props;
  ...
  const handleSubmit = async event => {
    ...
    setLoading(true);
    let res = await updateGoUser(user._id, { have_insurance: value === 'true' });
    setLoading(false);
    if (res.success) {
      nextStep(value === 'true');
    }
...
export default connect(null, { updateGoUser })(CheckAvailability);