...
import { updateGoPatient } from 'actions/api';
...
const Inputs = (props) => {
  const { user, patient, nextStep, backTab, updateGoPatient, insurance_information_required } = props;
  ...
  const handleChange = e => {
    ...
    setFormState({ ...formState, insurance: temp });
  };

  const handleSubmit = async event => {
    event.preventDefault();
    setLoading(true);
    const res = await updateGoPatient(user._id, formState);
    setLoading(false);
    if (res.success) {
      nextStep({
        ...formState,
        ...
        >
          <div className={classes.boxContainer}>
            <TextField
              ...
              fullWidth
              required={insurance_information_required}
...
Inputs.propTypes = {
  ...
  updateGoPatient: PropTypes.func.isRequired,
};

export default connect(null, { updateGoPatient })(Inputs);
