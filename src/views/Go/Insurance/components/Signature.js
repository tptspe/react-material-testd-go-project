...
const Question10 = (props) => {
  const { user, nextStep, backTab, uploadGoImage, setDisplayError, insurance_information_required } = props;
  ...
  const handleSubmit = async event => {
    if (!sigPad.isEmpty()) {
      ...
      setLoading(false);
      if (res.success) {
        nextStep({ signature: res.data });
      }
    } else {
      if ((user && user.signature) || !insurance_information_required) {
        nextStep();
      } else {
...
export default connect(null, { uploadGoImage })(Question10);
