...
const Question7 = (props) => {
  ...
  const handleSubmit = async event => {
    ...
    // to remove space between && else pass undefined for backend validation
    if (formState.zip_code && formState.zip_code.length)
      formState.zip_code = formState.zip_code.trim();
    else
      formState.zip_code = undefined;
    nextStep(formState);
  };
          ...
          <div className={classes.boxContainer}>
            ...
            {!address_hide &&
              <>
                <TextField
                  ...
                  fullWidth
                  required={address_required || home_address_required}
            ...
            {clientServer === 'prod007' &&
              <>
                ...
                <TextField
                  type="text"
                  label={t('question7_country')}
                  placeholder={t('question7_country')}
                  name="country"
                  className={clsx(brandClasses.shrinkTextField, classes.inputTextField)}
                  onChange={handleChange}
                  value={formState.country || ''}
                  fullWidth
                  required={home_address_required}
                  InputProps={{ classes: { root: classes.inputLabel } }}
                  inputProps={inputRegex}
                  InputLabelProps={{ shrink: true }}
                  variant="outlined"
