...
const Question8 = (props) => {
  const { user, submitLoading, nextStep, backTab, demographics_ssn_required } = props;
      ...
      <div className={classes.contentBody}>
        ...
        >
          ...
          /> */}
            <TextField
              type="number"
              label={clientServer === 'prod007' ? t('question8_ssn_label_prod007') : t('question8_ssn_label')}
              placeholder={clientServer === 'prod007' ? t('question8_ssn_place_prod007') : t('question8_ssn_place')}
              name="social_digits"
              className={clsx(brandClasses.shrinkTextField, classes.inputTextField)}
              onChange={handleChange}
              value={formState.social_digits || ''}
              fullWidth
              required={demographics_ssn_required}
